...
import { ArrowUpRight, Search, FileText, Upload, AlertCircle, RefreshCw } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ records, isLoading, onMinistryClick, onUploadRequest }) => {
  ...
  if (isLoading) {
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center animate-fadeIn">
            ...
            <p className="text-gw-muted text-sm">Checking for latest scraped records</p>
        </div>
    );
  }
  ...
  if (records.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center animate-fadeIn p-4">
        <div className="bg-gw-card border border-gw-border rounded-full p-6 mb-6 shadow-[0_0_30px_rgba(54,211,153,0.1)]">
          <Upload className="w-12 h-12 text-gw-success" />
        </div>
        <h2 className="text-2xl font-bold text-white mb-2">Database Empty</h2>
        <p className="text-gw-muted max-w-md mb-8">
          The scraper has not populated the data yet. You can wait for the automated job or upload a JSON file manually.
        </p>
        <button
          onClick={onUploadRequest}
          className="bg-gw-success hover:bg-gw-success/90 text-gw-bg font-bold py-3 px-8 rounded-lg transition-all flex items-center gap-2"
        >
  ...
  const ministryTotals = records.reduce((acc, curr) => {
    // Shorten ministry names for chart readability
    let shortName = curr.ministry.split('(')[1]?.replace(')', '') || curr.ministry.substring(0, 10);
    acc[shortName] = (acc[shortName] || 0) + curr.amount;
    return acc;
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-gw-card border border-gw-border rounded-lg overflow-hidden flex flex-col max-h-[800px]">
            ...
            <table className="w-full text-left text-sm relative">
              ...
              <tbody className="divide-y divide-gw-border">
                ...
                {filteredRecords.length === 0 && (
                    <tr>
                        <td colSpan={5} className="px-6 py-12 text-center text-gw-muted">
          ...
          <div className="p-3 border-t border-gw-border text-center text-xs text-gw-muted bg-gw-bg/50">
        ...
        <div className="bg-gw-card border border-gw-border rounded-lg p-6 flex flex-col">
          ...
          <p className="text-xs text-gw-muted mt-4 text-center italic">
            Ministries by total contract value (Red indicates top 3 spenders)
          </p>
        </div>
      </div>
    </div>
